...
function ListaLivros() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:5000/livros") // Faz uma requisição GET para a API para buscar a lista de livros
  ...
  const deletarLivro = (id) => {
    axios
      .delete(`http://localhost:5000/livros/${id}`) // Faz uma requisição DELETE para a API para remover o livro com o ID especificado
    ...
    <div>
      ...
      <ul>
        ...
        {livros.map((livro) => (
          <li key={livro._id}>
            {" "}
            {/* Cada item da lista tem uma chave única, o _id do livro */}
